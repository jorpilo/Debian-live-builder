...
if [ $? -ne 0 ]; then
 ...
 wget  -O /tmp/${repo[0]}.gz ${repo[1]}/dists/$Distro/main/binary-$Arquitecture/Packages.gz
 if [ $? -ne 0 ]; then
   dialog --title "Error" --msgbox "Couldn't download packages from ${repo[1]}" 0 0
   exit 1
 fi
 zcat /tmp/${repo[0]}.gz | awk '/Package:/{print $2}' > /tmp/${repo[0]}.txt
 zcat /tmp/${repo[0]}.gz | awk '/Description:/{$1 = ""; print $0}' > /tmp/${repo[0]}.des.txt
 sed -i "s/'//g" /tmp/${repo[0]}.txt
 sed -i 's/"//g' /tmp/${repo[0]}.txt
  sed -i "s/'//g" /tmp/${repo[0]}.des.txt
 sed -i 's/"//g' /tmp/${repo[0]}.des.txt
 ...
 if [ ! -f packages/${repo[0]}/${repo[0]}.packages ]; then
  ...
  while read -r package && read -r description <&3 ; do
      grep -w $package packages/${repo[0]}/${repo[0]}.packages
      if [ $? -eq 0 ]; then
          echo \""$package"\" \""$description"\" on >> /tmp/${repo[0]}.check
      else
          echo \""$package"\" \""$description"\" off >> /tmp/${repo[0]}.check
      fi
  done </tmp/${repo[0]}.txt 3</tmp/${repo[0]}.des.txt
